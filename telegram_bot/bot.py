...
async def command_start_handler(message: types.Message, state: FSMContext):
    ...
    text = (
        f"🛠️ Привет! Я бот системы Montazhka PRO.\n\n"
        f"Твой ID: `{chat_id}`\n"
...
async def receive_telegram_id(message: types.Message, state: FSMContext):
    ...
    if chat_id != telegram_id:
        await message.answer("🤔 Ты уверен, что это твой ID? Давай попробуем еще раз.")
        return
    ...
    await message.answer("👍 Отлично! Теперь введи email, который ты использовал при регистрации в Montazhka PRO.")
...
async def receive_email_and_update(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if response.data and len(response.data) > 0:
            ...
            await message.answer("🎉 Готово! Твой Telegram-аккаунт успешно привязан к системе.")
        else:
            print(f"🟡 Профиль с email {email} не найден.")
            await message.answer("🤷‍♂️ Не могу найти профиль с таким email. Попробуй еще раз или обратись в поддержку.")
